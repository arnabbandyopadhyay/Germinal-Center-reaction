...
box_width_lz=500
opseq=[3,3,3,3]
mum, sigm = 8, 1 # hr from LZ to DZ and vice versa
...
#    selected_cells=[b for b in LZ if b.tfhsignal > 1 and b.tfh_contact_end < tnow]
    ...
    igm_selected_cells=[b for b in LZ if b.antigen_collect > 0 and b.antigen_collect < 3 and b.tfhsignal > 0.5 and b.tfhsignal <= 1 and b.tfh_contact_end < tnow]
    igm_cells=[]
...
#    [LZ.remove(x) for x in igm_selected_cells]
    ...
    igm_cells.append([b for b in appcells if b.tfhsignal>0])
...
#    [igm_selected_cells.remove(x) for x in appcells]
    ...
    igg_selected_cells=[b for b in LZ if b.antigen_collect >= 3 and b.antigen_collect < 8 and b.tfhsignal > 1 and b.tfhsignal <= 1.5 and b.tfh_contact_end < tnow]
    igg_cells=[]
...
#    print(len(selected_cells))
    ...
    p_selected_cells=[b for b in LZ if b.antigen_collect >= 8 and b.tfhsignal > 1.5 and b.tfh_contact_end < tnow]
    ps_cells=[]
...
#    selected_cells=list(map(fun2,selected_cells))
    other=[b for b in LZ if b.antigen_collect>0 and b.tfhsignal>1.0 and b.tfh_contact_end < tnow]
    selected_cells= igm_selected_cells + igg_selected_cells + p_selected_cells + other
...
#    r.append([tnow,len(lists),len(LZ),len(outputcells),len(appopcells)])
    ...
    r[sum]=[tnow,len(lists),len(LZ),len(igm_cells),len(igg_cells),len(ps_cells),len(appopcells),np.mean([b.ndiv2 for b in lists]),np.mean([b.antigen_collect for b in LZ]),np.mean([b.tfhsignal for b in LZ]),np.mean([b.affinity for b in lists]),len([b for b in tlist if len(b.bcontact)<6])/len(tlist),atg_uptake]
