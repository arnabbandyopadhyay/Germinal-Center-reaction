...
##### antigen collection
files <- intersect(list.files(pattern = "antg_coll_LZ"),list.files(pattern = ".xlsx"))
#dat<-read_excel(paste0('antg_tfh/',files[2]), col_types = "numeric")
dat<-read_excel(files[2], col_types = "numeric")
dat$day<-rep(paste0('day',str_extract(files[2], "[0-9]+")),dim(dat)[1])
for (i in 3:20){
#  dat2<-read_excel(paste0('antg_tfh/',files[i]), col_types = "numeric")
  dat2<-read_excel(files[i], col_types = "numeric")
  dat2$day<-rep(paste0('day',str_extract(files[i], "[0-9]+")),dim(dat2)[1])
  dat<-rbind(dat,dat2)
}

dat$day_f = factor(dat$day, levels=c('day2','day3','day4','day5','day6',
                                     'day7','day8','day9','day10','day11',
                                     'day12','day13','day14','day15',
                                     'day16','day17','day18','day19','day20'))

pdf('antigen_distribution_per_day_LZ.pdf')
p <- ggplot(dat, aes(x=dat$`0`)) + 
  geom_density(aes(fill=dat$day))+facet_wrap( .~ dat$day_f, scales = "free")+
  theme(legend.position = "none")+labs(x = "")
p
dev.off()
##### tfh signal collection
#files <- list.files('antg_tfh/',pattern = "tfh_")
files <- intersect(list.files(pattern = "tfh_LZ"),list.files(pattern = ".xlsx"))
#dat<-read_excel(paste0('antg_tfh/',files[2]), col_types = "numeric")
dat<-read_excel(files[2], col_types = "numeric")
dat$day<-rep(paste0('day',str_extract(files[2], "[0-9]+")),dim(dat)[1])
for (i in 3:20){
...
files <- intersect(list.files(pattern = "antg_aff_LZ"),list.files(pattern = ".xlsx"))
#dat<-read_excel(paste0('antg_tfh/',files[2]), col_types = "numeric")
dat<-read_excel(files[2], col_types = "numeric")
dat$day<-rep(paste0('day',str_extract(files[2], "[0-9]+")),dim(dat)[1])
for (i in 3:20){
...
############## antigen collection per day high signal

files <- intersect(list.files(pattern = "antg_coll_high"),list.files(pattern = ".xlsx"))
#dat<-read_excel(paste0('antg_tfh/',files[2]), col_types = "numeric")
dat<-read_excel(files[2], col_types = "numeric")
dat$day<-rep(paste0('day',str_extract(files[2], "[0-9]+")),dim(dat)[1])
for (i in 3:20){
...
pdf('antigen_collection_per_day_high.pdf')
p <- ggplot(dat, aes(x=dat$`0`)) + 
  geom_density(aes(fill=dat$day))+facet_wrap( .~ dat$day_f, scales = "free")+
  theme(legend.position = "none")+labs(x = "")
p
...
dat2<-read_excel(files[7], col_types = "numeric")
d<-density(dat2$`0`)
plot(d)


##################output files multi-run

####### gc dynamics
#files <- list.files(recursive = TRUE,pattern = "output.txt$")
files<-intersect(list.files(pattern = "output"), list.files(pattern = ".txt"))
dat<-read.csv(files[1], sep=' ')
dat2<-dat[,c(1,2)]
cols<-c(2,3)
colnames1<-c('mean_DZ','Mean_LZ')
colnames2<-c('SD_DZ','SD_LZ')
nd<-data.frame(time=dat2[,1])
k=NULL
k1=NULL
sum=0
for (j in cols){
  ...
  for (i in 2:length(files)){
    ...
    dat2<-cbind(dat2,dat[,j])
    
  }
  k<-data.frame(rowMeans(dat2[,c(2:11)]))
  names(k) <- colnames1[sum]
  k1<-data.frame(sd=apply(dat2[,c(2:11)],1,sd))
  names(k1) <- colnames2[sum]

  nd<-cbind(nd,k,k1)
  
}
nd<-nd[-50000,]
nd$total<-nd$mean_DZ+nd$Mean_LZ
gc<-nd[,c(1,2,4,6)]
names(gc)<-c('time','DZ B cells','LZ B cells','Total B cells')
gc<-melt(gc,id=c('time'))
# plot<-ggplot(data=nd,aes(x=nd$time,y=nd$mean_DZ))+
#   geom_line(color='red') + 
#   geom_line(data = nd, mapping =aes(x=nd$time,y=nd$Mean_LZ))#+
# #  geom_errorbar(aes(ymin=nd$mean_DZ-nd$SD_DZ, ymax=nd$mean_DZ+nd$SD_DZ))
# plot
pdf('gc_dynamics_mem.pdf')
gc$time<-gc$time/24
ggplot(data=gc, aes(x=time, y=value, color=variable)) + geom_line(size = 1)+
  labs(x = "days",y='B cells')
...
dat2<-dat[,c(1,4)]
cols<-c(4,5,6)
colnames1<-c('mean_igm','Mean_igg','Mean_plasma')
colnames2<-c('SD_igm','SD_igg','sd_plasma')
#nd<-data.frame(time=dat2[,1])
nd<-data.frame(time=seq(1.5,500.5,1))
sum=0
for (j in cols){
  sum=sum+1
  print(sum)
  ...
  k1<-seq.int(1,500,1)
  result <- data.frame(matrix(nrow = length(k1), ncol = 3))
  for (ii in 1:length(k1)){
    print(ii)
    dd<-dat2[dat2[,1]>k1[ii] & dat2[,1]<k1[ii+1],]
    result[ii,1]<-colMeans(dd)[1]
    result[ii,2]<-mean(colSums(dd)[c(2:11)])
    result[ii,3]<-sd(colSums(dd)[c(2:11)])
    
    
  }
  nd<-cbind(nd,result$X2,result$X3)
  
  
}
nd<-nd[-500,]
names(nd)<-c('time','igm','sd_igm','igg','sd_igg','plasma','sd_plasma')
newdat<-nd[,c(1,2,4,6)]
newdat<-melt(newdat,id=c('time'))
newdat$time<-newdat$time/24

# plot<-ggplot(data=nd,aes(x=nd$time/24,y=nd$mean_igm))+
#   geom_line(color='red') + 
#   geom_line(data = nd, mapping =aes(x=nd$time/24,y=nd$mean_igg),color='blue')+
#   geom_line(data = nd, mapping =aes(x=nd$time/24,y=nd$mean_plasma),color='green')
#   
# plot+labs(x = "days",y='production/hr')+
#    scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"), 
#                       name="Experimental\nCondition",
#                       breaks=c("mean_igm", "mean_igg", "mean_plasma"),
#                       labels=c("Control", "Treatment 1", "Treatment 2"))

pdf("ig_dynamics.pdf") 
ggplot(data=newdat, aes(x=time, y=value, color=variable)) + geom_line(size = 1)+
...
dat<-read.csv(files[1], sep=' ')
dat2<-dat[,c(1,8)]
dat2<-dat2[!is.na(dat2$nan),]

p<-ggplot(data=dat2,aes(x=dat2$X1.0,y=nan))+geom_line(color='red')
p

  for (i in 2:length(files)){
    
    dat<-read.csv(files[i], sep=' ')
    dat2<-cbind(dat2,dat[,8])
    
  }
  k<-data.frame(time=dat2[,1],div=rowMeans(dat2[,c(2:11)]))
k<-k[-50000,]
p<-ggplot(data=k,aes(x=time,y=div))+geom_line(color='red')
p
...
dat<-read.csv("output5.txt", sep=' ')
dat<-read.csv("tfh_2.dat", sep=',')
dat<-dat[-50000,]
my_plot <- ggplot(data = dat, 
                  mapping = aes(x = dat$X1.0, y = dat$X5+dat$X0)) +
  geom_line(color='red') + geom_line(data = dat, 
                                     mapping = aes(x = dat$X1.0, y = dat$X5),color='blue')+ geom_line(data = dat, 
                                                                                                      mapping = aes(x = dat$X1.0, y = dat$X0),color='green')
my_plot

my_plot <- ggplot(data = dat, mapping = aes(x = dat$X1.0, y = dat$X0.002039879900127865)) +geom_line(color='red')
my_plot
...
for (i in 1:length(k1)){
  print(c(k1[i],k1[i+1]))
  dd<-igm[igm$X1.0>k1[i] & igm$X1.0<k1[i+1],]
