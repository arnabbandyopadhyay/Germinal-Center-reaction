...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 12:18:55 2020
...
class Bcell:
    ...
    def __init__(self, birthtime, klass, **kwargs):
        ...
        if 'sequence' in kwargs: self.sequence = kwargs['sequence']
            ...
            self.sequence = np.random.randint(8,10,4).tolist()
        elif klass=='igg':
        ...
        else: self.ndiv = np.random.randint(2,3)
        
        if 'ndiv2' in kwargs: self.ndiv2 = kwargs['ndiv2']
        else: self.ndiv2 = np.random.randint(2,3)
...
pmax=6
kp=12
lists=[Bcell(0,'igg'),Bcell(0,'igg'),Bcell(0,'igm')]
...
if __name__ == '__main__':
    ...
    with Pool(5) as p:
        ...
        #    ps_cells=[b for b in p_selected_cells if b.affinity > 0.6]
            ps_cells=[]
...
#            print([b for b in selected_cells])
            selected_cells2=list(p.map(fun2,selected_cells))
#            print([b for b in selected_cells2])
        ...
        #    r.append([tnow,len(lists),len(LZ),len(outputcells),len(appopcells)])
            ...
            r[summ]=[tnow,len(lists),len(LZ),len(lists)+len(LZ),len(igm_cells),len(igg_cells),len(ps_cells),\
             ...
             avg_div,avg_div2,atg_uptake,np.mean([b.tfhsignal for b in LZ]),np.mean([b.affinity for b in lists]),len([b for b in tlist if len(b.bcontact)<6])]
        ...
        #    print(tnow,len(lists),len(LZ),len(igm_cells),len(igg_cells),len(ps_cells),len(appopcells),avg_div,avg_div2,atg_uptake,np.mean([b.tfhsignal for b in LZ]),np.mean([b.affinity for b in lists]),len([b for b in tlist if len(b.bcontact)<6]))
            ...
            if (round(tnow,2) % 50) == 0:
                df = pd.DataFrame(r)
                df.columns = ['time', 'DZ', 'LZ','DZ+LZ','igm','igg','ps',\
                              'igm_in_igm','igg_in_igm','igm_in_igg','igg_in_igg','igm_in_ps','igg_in_ps',\
                              'aff_igm_in_igm','aff_igg_in_igm','aff_igm_in_igg','aff_igg_in_igg','aff_igm_in_ps','aff_igg_in_ps',\
                              'avg_div','avg_div2','antigen_uptake','mean tfh signal LZ','mean affinity DZ','free Tfh']
                writer = pd.ExcelWriter('output_mem6.xlsx', engine='xlsxwriter')
